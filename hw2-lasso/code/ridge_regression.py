...
def compare_our_ridge_with_sklearn(X_train, y_train, l2_reg=1):
    ...
    ridge_regression_estimator = RidgeRegression(l2reg=l2_reg) #Adam: I don't really understand how this works!
...
def do_grid_search_ridge(X_train, y_train, X_val, y_val):
    ...
    val_fold = [-1]*len(X_train) + [0]*len(X_val) #0 corresponds to validation #Adam: [-1] is an index for training

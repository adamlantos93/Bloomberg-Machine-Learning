...
class MLPRegression(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        init_values = {
        "W1": s * np.random.rand( self.num_hidden_units, num_ftrs),
        "b1": s * np.random.rand(self.num_hidden_units),
        "W2": s * np.random.rand(self.num_hidden_units),
        "b2": s * np.array(np.random.rand()),
        }       

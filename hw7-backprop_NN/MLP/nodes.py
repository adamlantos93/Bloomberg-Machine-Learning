...
class SquaredL2DistanceNode(object):
    ...
    def backward(self):
        ...
        #print('AAAA',type(self.a.out), type(self.b.out))
        #print(d_b.shape, self.b.d_out.shape)
        #print(d_a.shape, self.a.d_out.shape)
        #print(np.random.rand(2), type(np.random.rand(2)))
        #print(np.array(np.random.rand()), type(np.array(np.random.rand())))
        self.a.d_out += d_a
        self.b.d_out += d_b
        return self.d_out
...
class L2NormPenaltyNode(object):
    ...
    def forward(self):
        self.out = self.l2_reg*np.dot(self.w.out, self.w.out)  #result of node
        self.d_out = np.zeros(self.out.shape) #so that it doesn't play a role in forward(x), but can change it for backward
        return self.out
    ...
    def get_predecessors(self):
        return [self.w]   #don't return  self.l2_reg, as it's not a node but a scalar
...
class TanhNode(object):
    ...
    def backward(self):
        d_a= self.d_out * (1-self.out**2)  #Hadamard (element-wise mutliplication) product for the derivative of function of matrix
        self.a.d_out += d_a
